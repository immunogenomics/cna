...
from ._nam import _df_to_array, nam


def _association(
    NAMsvd, M, r, y, batches, ks=None, Nnull=1000, local_test=True, seed=None
):
    if seed is not None:
        np.random.seed(seed)
    ...
    if k == max(ks):
        warnings.warn(
            ...
            f'Consider allowing more PCs by using the "ks" argument.'
        )
    ...
    if (nullminps <= p + 1e-8).sum() == 0:
        warnings.warn(
            f"global association p-value attained minimal possible value. "
            f"Consider increasing Nnull"
        )
...
def association(
    ...
    **kwargs,
    ...
    if y.shape != (data.N,):
        raise ValueError(
            "y should be an array of length data.N; instead its shape is: "
            + str(y.shape)
        )
