...
def diffusion(a, Y, C=None, B=None, T=None, s=None,
    ...
    1d array: the set of timepoints corresponding to the returned results
    """
    def corr(g,h):
    ...
    tokeep = set([
        i for i in range(1, maxsteps+1, keepevery)
        ])
    d = np.zeros((len(tokeep), len(y)))
    z = np.zeros((len(tokeep), len(y)))
    bonf_z2 = np.zeros(len(tokeep))
    stds = np.zeros((len(tokeep), len(y)))
    ...
    for i in range(1, maxsteps+1):
        ...
        if i in tokeep:
            j = int((i-1) / keepevery)
            d[j] = dcurr
            stds[j] = np.sqrt((dnull**2).mean(axis=1))
            z[j] = dcurr / stds[j]
            bonf_z2[j] = np.percentile(
                np.max((dnull / stds[j][:,None])**2, axis=0),
                95)
    ...
    mlp = -(st.norm.logsf(np.abs(z)/np.log(10) + np.log10(2)))

    return d, z, bonf_z2, stds, mlp, np.array(sorted(tokeep))
