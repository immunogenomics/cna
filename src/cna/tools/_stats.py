...
def conditional_permutation(B, Y, num):
    ...
    """
    batchind = np.array([np.where(B == b)[0] for b in np.unique(B)])
    ix = np.concatenate(
        [bi[np.argsort(np.random.randn(len(bi), num), axis=0)] for bi in batchind]
    )
...
def numtests_loo(Nmaxz2):
    return np.array(
        [numtests(Nmaxz2[np.arange(len(Nmaxz2)) != i]) for i in range(len(Nmaxz2))]
    )
