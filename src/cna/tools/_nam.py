...
def _batch_kurtosis(NAM, batches):
    return (
        st.kurtosis(
            np.array([NAM[batches == b].mean(axis=0) for b in np.unique(batches)]),
            axis=0,
        )
        + 3
    )
...
def _resid_nam(NAM, covs, batches, ridge=None):
    ...
    if batches is None or len(np.unique(batches)) == 1:
        ...
        for ridge in ridges:
            ...
            print(
                "\twith ridge", ridge, "median batch kurtosis = ", np.median(kurtoses)
            )
...
def nam(
    ...
    **kwargs
    ...
    if (
        force_recompute
        or "NAM.T" + suffix not in du
        or not safe_same(batches, du["_batches" + suffix])
    ):
        print("qcd NAM not found; computing and saving")
        NAM = _nam(data, nsteps=nsteps)
        NAMqc, keep = _qc_nam(NAM.values, batches)
        du["NAM.T" + suffix] = pd.DataFrame(
            NAMqc, index=NAM.index, columns=NAM.columns[keep]
        ).T
    ...
    if (
        ...
        du["NAM_sampleXpc" + suffix] = pd.DataFrame(
            U, index=NAM.index, columns=["PC" + str(i) for i in range(1, len(U.T) + 1)]
        )
